...
public class DetailActivity extends SwipeRefreshActivity implements OnClickListener {
		...
		try {
			...
			h.postDelayed(new Runnable() {
				...
				public void run() {
					...
					imgViewMap.setOnClickListener(new View.OnClickListener() {
	...
	private void updateStore() {
		...
		viewPager=(ViewPager)findViewById(R.id.view_pager_photo);
	...
	// Photo Adapter View Pager
	class PhotoPageAdapter extends PagerAdapter {

		@Override
		public int getCount() {
			Log.d("tag","Photo Size "+photoList.size());
		...
		public Object instantiateItem(ViewGroup container, int position) {
			...
			MainActivity.getImageLoader().displayImage(photoList.get(position).getPhoto_url(), imageView, options);
	...
	void showPhoto()
	{
		...
		if(photoList != null && photoList.size() > 0) {					
			Intent i1 = new Intent(this, ImageViewerActivity.class);
			i1.putExtra("photoList", photoList);
			startActivity(i1);
		}
		else {
			MGUtilities.showAlertView(
